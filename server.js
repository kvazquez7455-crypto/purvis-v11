...
app.get('/api/qa', async (req, res) => {
  ...
  const tests = [
    ...
    { name: 'features', test: async () => { const { error } = await supabase.from('purvis_feature_flags').select('feature').limit(1); return !error; } },
    ...
    { name: 'agent_controls', test: async () => { const { error } = await supabase.from('purvis_agent_controls').select('job_name').limit(1); return !error; } },
